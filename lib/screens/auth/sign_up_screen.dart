...
import 'package:firebase_auth/firebase_auth.dart' hide AuthProvider; // Import FirebaseAuthException
...
class _SignUpScreenState extends State<SignUpScreen> {
  ...
  Future<void> _signUpWithEmailPassword() async {
    ...
    try {
      ...
      final success = await authProvider.signUpWithEmailPassword(email, password);
      ...
      if (success) {
        ...
        if (mounted) _showErrorSnackbar('Sign up failed. Please try again.'); // Generic message if no exception but still failed
      }
    } on FirebaseAuthException catch (e) {
       print('SignUpScreen: FirebaseAuthException code: ${e.code}');
       String errorMessage = 'An error occurred during sign up. Please try again.'; // Default message
                ...
                children: [
                  ...
                  TextFormField(
                    ...
                    validator: (value) {
                      ...
                      if (value.length < 6) return 'Password must be at least 6 characters'; // Example length check
                  ...
                  ElevatedButton(
                    ...
                    onPressed: authProvider.isSigningIn ? null : _signUpWithEmailPassword,
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      const Text("Already have an account?", style: TextStyle(color: darkGrey)),
                      TextButton(
                        onPressed: () => Navigator.of(context).pop(), // Go back to LoginScreen
                        child: const Text('Login', style: TextStyle(color: primaryGreen, fontWeight: FontWeight.bold)),
                      ),
                    ],
                  ),
                ],
